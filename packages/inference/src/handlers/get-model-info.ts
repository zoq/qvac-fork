...
export async function handleGetModelInfo(
  ...
  const catalogEntry: RegistryItem | undefined = models.find((m) => m.name === name)
...
async function handleCompanionSetModel(
  ...
  const canonicalResult = await checkCacheStatus(fileEntries, canonicalPaths, primaryKey)
  if (canonicalResult.isCached) return canonicalResult

  const isOnnxSet = files.some((f) => f.primary === true && f.targetName.endsWith('.onnx'))
  if (isOnnxSet) {
    ...
    const legacyResult = await checkCacheStatus(fileEntries, legacyPaths, primaryKey)
    ...
    const flatPaths = files.map((f) => getSingleFileCachePath(f.registryPath))
    const flatResult = await checkCacheStatus(fileEntries, flatPaths, primaryKey)
...
async function handleSingleFileModel(
  ...
  return checkCacheStatus([{ filename, expectedSize, sha256Checksum }], [filePath])
}
