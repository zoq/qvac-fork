...
import { DownloadCancelledError, RegistryDownloadFailedError } from '@/errors/index'
...
export async function downloadCompanionSetFromRegistry(
  ...
  try {
    for (let i = 0; i < files.length; i++) {
      ...
      const blobBinding = file.blobCoreKey ? buildBlobBinding(file) : undefined
...
function validateCompanionSetMetadata(
  ...
  if (!files.some((f) => f.key === primaryKey)) {
    throw new RegistryDownloadFailedError(`Companion set missing primary file (key: ${primaryKey})`)
  }
...
function isLegacyOnnxSet(files: readonly CompanionSetMetadataEntry[]): boolean {
  return files.some((f) => f.primary === true && f.targetName.endsWith('.onnx'))
}
