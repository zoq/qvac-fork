...
test('Whisper prompt reload strips engine-only keys for asr-ggml', (t) => {
...
test('ASR event adapters preserve the public engine event contract', (t) => {
  t.ok(
    isVadEvent({
      type: 'vad',
      speaking: true,
  ...
  t.absent(isVadEvent({ text: 'vad-like segment', type: 'vad' }))
  t.alike(
    toVadStateEvent({
      type: 'vad',
      speaking: true,
