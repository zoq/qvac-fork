...
test('assertMetadataSupported: throws TranscriptionFailedError for parakeet', (t) => {
  ...
  t.ok(String((caught as Error).message).includes('metadata'), 'error message mentions metadata')
  t.ok(
...
test('duplex metadata integration: end-to-end segments from audio chunks', async (t) => {
  ...
  t.ok(segments[1]!.endMs > segments[1]!.startMs, 'second segment endMs > startMs')
})
