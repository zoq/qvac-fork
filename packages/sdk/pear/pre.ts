...
function ensureObjectProp(obj: Record<string, unknown>, prop: string): Record<string, unknown> {
...
function normalizePearWorkerPath(value: unknown): string {
  ...
  if (p === '.' || p === '..' || p.startsWith('../') || p.includes('/../')) {
    throw new Error(
      `"pearWorker" must be a path inside the app root (got: ${JSON.stringify(value)})`
    )
  }
  return p
}
...
function getAppRoot(): string {
  ...
  if (url.protocol !== 'file:') {
    throw new Error(`Expected Pear.config.applink to be a file:// URL (got: ${applink})`)
  }
...
function parseBuiltinSpecifier(specifier: string): { suffix: string; exportName: string } | null {
...
function generatePearWorkerEntry(plugins: string[], appWorkerPath: string): string {
  ...
  for (const specifier of plugins) {
    ...
    if (builtin) {
      imports.push(`import { ${builtin.exportName} } from "${SDK_NAME}/${builtin.suffix}/plugin";`)
